...
1. Read this file, `docs/PROGRESS.md` and your milestone's contract (see **Milestones in worktrees** below): `docs/M8.10_PLAN.md`, the island (its design in its §1, its sketches in `docs/island/`), and `docs/M8.9_PLAN.md`, the look (each its design and contract in one file); `docs/M8.8_PLAN.md`, the fleet, whose session has finished its work (Marcin, 2026-09-25); then `docs/M9_PLAN.md`, the platform, after the gates, only on Marcin's word; the older design in `docs/DESIGN.md`. M8.7's gate closed at 0.8.7 (`docs/M8.7_REPORT.md`). Check `git log --oneline -15`.
...
## Milestones in worktrees (Marcin, 2026-09-25)

Each milestone is built by its own session in its own git worktree and branch; M9 starts after their gates. M8.8 "The fleet" (`docs/M8.8_PLAN.md`) and M8.9 "The look" (`docs/M8.9_PLAN.md`) began side by side; the fleet's session has finished its work (Marcin, 2026-09-25) and M8.10 "The island" (`docs/M8.10_PLAN.md`) took its place beside the look. Where a plan says it waits for another's gate, this section decides: they run side by side, and the handovers of rule 4 replace the waiting.

| | M8.8 the fleet (finished: no gate, its branch deleted, Marcin 2026-09-25) | M8.9 the look | M8.10 the island |
|---|---|---|---|
| Session | "Pojazdy w grze" | "UI/UX i style" | the island's |
| Branch | `m8.8-fleet` (worktree `.claude/worktrees/m8.8-fleet`) | `m8.9-ui` (worktree `.claude/worktrees/m8.9-ui`) | `m8.10-island` (worktree `.claude/worktrees/m8.10-island`) |
| Owns | the sim (vehicles, bodies, traffic, police, the ground and the sea, jobs), the cars' and the traffic's meshes, the engine voices | how the screen and the world look: `src/ui/` (styles, HUD, wall, maps), the light, the sky, the city's colours, the signs' look, the showroom and its pictures | the island (`src/sim/island/`, `src/render/island/`, the atlas `tools/atlas.*`), everything the switch to it moves (its plan's §1 and §3), and, since the fleet finished, the sim the island needs |

1. **Nobody works in the main folder** (`C:\Games\Nowy folder (6)`): it is Marcin's, who plays there (`git pull`, then `npm start`). Each session works in its own worktree (`npm ci` there once) and never touches the other's or the main folder (a worktree session's git is kept inside its worktree anyway).
2. **A slice reaches main through origin, whole**: commit on the branch; `git fetch origin`; `git merge origin/main` into the branch and resolve there; `npm run verify` green; `git push origin HEAD:main` (a fast-forward; if it is rejected because the other got there first, fetch, merge and verify again). No `git add -A`; nothing else is pushed.
3. **Shared files are added to, not rewritten**: `src/ui/pl.ts`, `tests/ui/words.test.ts`, `tests/ui/lang.test.ts`, `src/sim/palette.ts`, `src/sim/glyphs.ts`, `docs/PROGRESS.md` (entries at the top, newest first), `docs/BACKLOG.md`, `docs/STYLE.md`; a conflict keeps both sides. Before a slice that touches a file the other also changes (`src/ui/wall/garage.ts`, `src/ui/map/*`, `src/render/cars/PlayerCar.ts`, `src/render/cars/bodyMesh.ts`, `src/render/camera/CameraDirector.ts`, `src/render/run/MarkerView.ts`, `src/render/city/CityView.ts`, `src/app/App.ts`, `src/audio/Sfx.ts`), fetch and merge `origin/main` in first.
...
6. **Servers, measurements, gates.** 4173 is Marcin's game (`npm start`). The e2e suites, the quick verify's smoke and M8.9's stills included, serve each worktree's build on its own port, derived from its folder's name (`e2e/port.ts`: `m8.8-fleet` 4291, `m8.9-ui` 4238, `m8.10-island` 4283; the main folder keeps 4173), and a busy port fails the run instead of testing another build; so a worktree is named as in the table above. The perf, the e2e suites, the look gate, the balance and a gate run only when Marcin says the other session is idle, the gates one at a time. M8.7's unrun perf A/B, e2e suites and balance (its report) fall to the first gate. Each gate's perf is an A/B against 0.8.7 (`52db2bc`), and against the other's gate build if that closed first. A gate sets `package.json` to its own version unless a higher one is on main.
7. **CLAUDE.md** is edited by the island's session (Marcin, 2026-09-25: the fleet's, which held it, has finished); the look's session passes its lines (its gate's colour table and typeface) to Marcin, who hands them over.
...
| `docs/M8.8_PLAN.md` | The fleet: its design and its contract in one document (Marcin's request, 2026-09-24): a job for every class, trophies each best at one thing, the 4×4 and the ground, three crazy cars, the motorbike, the hovercraft and the sea, the mega-ramp, the police and the rivals on the car model. Beside M8.9 and before the platform (Marcin, 2026-09-25; **Milestones in worktrees**); its session has finished its work. |
| `docs/M8.9_PLAN.md` | The look: its design and its contract in one document, from a review of every screen that Marcin accepted with his bar (6/10 now, 9 at least): the world at a real golden hour and lit at dusk, one colour for one meaning, one typeface in two styles, a HUD that scales with the screen, one message at a time, a radar that answers three questions, signs that read from afar and never block, the garage as a showroom. Beside M8.8, then M8.10 (Marcin, 2026-09-25; **Milestones in worktrees**); its slice 0 first. |
| `docs/M8.10_PLAN.md` | The island: its design (§1) and contract (slices 0–19) in one file, from Marcin's verdict on the grid island's atlas ("a simple generator with a few authored roads") and the sketches he took whole: the plan drawn by hand, the rest from the seed; the game's world since its switch (slice 18, 2026-09-26), made from its bake (`npm run bake`, run by `npm run build`), the grid behind `?map=grid` until the gate. Beside M8.9 (Marcin, 2026-09-25; **Milestones in worktrees**). |
| `docs/island/` | The island's two sketches Marcin accepted: `plan.jpg` (the island) and `placed.jpg` (what stands where). `npm run atlas` draws the island as the sim builds it, to compare. |
| `docs/M9_PLAN.md` | The platform contract (SDK adapter, touch, mobile tier, submission), the brief's M6, after the M8.9 and M8.10 gates on Marcin's word (M8.8 closed without one). |
...
The island's plan is drawn by hand, the rest comes from the seed (Marcin, 2026-09-25, `docs/M8.10_PLAN.md` §1; the brief's "generated procedurally from a seed" changed): the coast, the heights, the districts, the main roads and every placed thing live in `src/sim/island/plan.ts`; the districts' own streets, the lots, the buildings and the props are generated. Its names come true (the hill under Crown Heights, the palms in Palm Gardens, the quay and the reef in Coral Quay) and no street ends in a field. The world's axes are +X west and +Z north (the full map draws −x right, −z down).
