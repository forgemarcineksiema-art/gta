...
| `docs/M8.10_PLAN.md` | The island: its design (§1) and contract (slices 0–19) in one file, from Marcin's verdict on the grid island's atlas ("a simple generator with a few authored roads") and the sketches he took whole: the plan drawn by hand, the rest from the seed, built behind `?map=island` until its switch. Beside M8.9 (Marcin, 2026-09-25; **Milestones in worktrees**). |
