...
1. Read this file, `docs/PROGRESS.md` and the current milestone contract (M7 at its gate, `docs/M7_REPORT.md`; then `docs/M8_PLAN.md`, the chaos, only on Marcin's word; the design in `docs/DESIGN.md`, §16 first); check `git log --oneline -15`.
...
| `docs/M7_PLAN.md` | The last milestone's contract, at its gate (`docs/M7_REPORT.md`, 0.7.0): the polish (DESIGN §15). M8 starts only on his word. |
| `docs/M8_PLAN.md` | The next contract, after his 0.7.0 playtest and on his word: the chaos (DESIGN §16): street furniture with mass, the knock decided before the physics step, each district's things, the smash as a skill-chain trick, the bill at the door. Slices in order, done criteria, pins. |
...
| `docs/M9_PLAN.md` | The platform contract (SDK adapter, touch, mobile tier, submission), the brief's M6, after M8 on Marcin's word. |
