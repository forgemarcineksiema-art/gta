...
1. Read this file, `docs/PROGRESS.md` and your milestone's contract: `docs/M8.10_PLAN.md`, the island (its design in its §1, its sketches in `docs/island/`), and `docs/M8.9_PLAN.md`, the look (each its design and contract in one file); `docs/history/M8.8_PLAN.md`, the fleet, whose session has finished its work (Marcin, 2026-09-25); then `docs/M9_PLAN.md`, the platform, after the gates, only on Marcin's word; the older design in `docs/DESIGN.md`. M8.7's gate closed at 0.8.7 (`docs/history/M8.7_REPORT.md`). Check `git log --oneline -15`.
...
| `docs/history/` | Everything finished: the plans and gate reports of M1 to M8.8 (`M<n>_PLAN.md`, `M<n>_REPORT.md`, kept for their numbers and as-built notes; the last gate is 0.8.7, `M8.7_REPORT.md`; M4_PLAN §5 holds the contract for post-launch update 1), the reviews and the archived session logs (`PROGRESS_*.md`). |
...
- Gate report at each milestone: what was built, verify and perf numbers, how to run, a five-minute playtest script, tuning knobs and where they live, known issues, proposed next scope. Each gate's perf is an A/B against 0.8.7 (`52db2bc`) and the previous gate's build; M8.7's unrun perf A/B, e2e suites and balance fall to the first gate; a gate sets `package.json` to its own version. A slice that needs a new save version takes the number after the one on main.
- The main folder is where Marcin plays (`git pull`, then `npm start` on 4173).
