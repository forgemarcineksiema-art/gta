...
src/render/           Three.js, reads the sim only. The root is the frame: Renderer (WebGL, lights, sky, the views in order), quality, shadows, geometry (shared prisms)
  camera/             ChaseCamera (look-ahead, reverse orbit, shake, whip, focus, cuts, occlusion), fade (a car in the camera's way thinned)
  cars/               every car's shape: carMesh (the loft, dents, wheels), carProfiles (the player's classes), bodyProfiles + bodyMesh (the city's bodies), trafficMesh (the classes as instanced cruisers), kitMesh (toppers, neon, flames, spoilers)
  ...
  fx/                 Sparks, Smoke, Debris, SkidMarks, SpeedLines
