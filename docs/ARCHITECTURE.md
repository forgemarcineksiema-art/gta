...
33. **The cold open is a script over the normal game** (M4 slice 4). `ColdOpen` never writes controls: it puts the player in a van at heat 1, holds a muscle car alongside with a guide plan, lays a coin line and a job marker, and reads the ring. Verbs are done in any order and the caption is the first verb not done while its cue holds, so a player who smashed the gate before swapping is never asked to; four verbs give up on their own. The session flag is set when it starts ("never shown twice").
