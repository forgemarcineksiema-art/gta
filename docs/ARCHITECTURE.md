...
src/render/           Three.js, reads the sim only. The root is the frame: Renderer (WebGL, the views in their order, the events), quality (tiers, AutoQuality), sky (dome, fog, sun), shadows, shapes (the sim's statics and dynamics, placing from the transform buffer), geometry (shared prisms)
  camera/             ChaseCamera (look-ahead, reverse orbit, shake, whip, focus, cuts, occlusion), CameraDirector (the swap whip, the door cut, the takedown's side cut), fade (a car in the camera's way thinned)
  cars/               every car's shape: PlayerCar (the classes and taken bodies, the swap, resprays, the crumple, the kit), GhostCar, carMesh (the loft, dents, wheels), carProfiles (the player's classes), bodyProfiles + bodyMesh (the city's bodies), trafficMesh (the classes as instanced cruisers), kitMesh (toppers, neon, flames, spoilers)
  city/               CityView (chunks, detail levels), skyline, roadPaint, Billboards, SignalView, BreakerView, RampView
  props/              propMesh (each prop's parts), PropsView (the knocked ones)
  traffic/            TrafficView (the city's cars), PedView + pedMesh (the crowd)
  police/             PoliceView (livery, light bars), HeliView, RoadblockView, DonutShop
  run/                the run's marks on the city: Arrow, MarkerView, Coins, HideoutView (doors, garage, rivals' board)
  fx/                 Effects (what each event throws, the bonnet and wreck smoke), Sparks, Smoke, Debris, SkidMarks, SpeedLines
src/audio/            EngineAudio (WebAudio synthesis), Sfx (event one-shots: sweep, honk, yelp, crunch, boom, whoosh, splinter, chime)
src/ui/               DOM over the canvas, reads the sim only. The root holds what every screen uses: lang + pl (every word on the screen, DESIGN §19), styles.css; and the pause screen's settings
  hud/                the driving screen: hud (the corners' DOM, popups, cards), corners (what each corner shows), heat (the stars), voice (one voice per event), lanes (the top centre's order), jobs (the goal line and its cards), run (bag, multiplier, bank), coldOpen (the captions), totals (the door's and the busted card's sums)
  map/                minimap (heading-up canvas radar) + minimapModel (pure maths), bigmap (the full map)
  wall/               garage (the wall behind the door, four pages), wallPages, wallGrid (the keys as a grid)
  dev/                debugPanel (live tuning), telemetryGraph
...
`render/city/CityView.ts` generates merged typed-array geometry directly (the city's
...
(`ui/map/minimap.ts`, maths in `ui/map/minimapModel.ts`): roads from the graph cached as
...
The skyline layer (`render/city/skyline.ts`) is one mesh from `City.landmarkSilhouettes`
...
their projected thickness reaches a pixel (`render/city/roadPaint.ts`): the same
...
onto its lane when its body is returned. Since M8.6 a driving body's roll
and pitch are locked too (record 90).
...
- `ui/dev/telemetryGraph.ts`: 10 s scrolling traces in the dev panel.
- `sim/vehicle/presets.ts` + `render/cars/carProfiles.ts`: vehicle classes as tuning overrides and loft profiles under one id; `tests/sim/cars.test.ts` pins each class.
...
47. **The garage spends the bank and the coins** (M5 slice 4; one pool since M8.5, record 88). DESIGN.md §3.3's first car counts both; `Run.funds` is their sum and `spend` takes the bank first. The pools stay apart (D14): coins never enter the bank or the bag.
...
86. **Everyone knocks by the one rule; the walkers step after the knocks** (M8 slice 7, D6). `Props.step` sweeps the player, then every lent body near the player at its body's velocity (a parked record's speed is stale) with its own mass, the Δv on its body; a car on its lane (a record) only ploughs lying props aside. The knocker is passed down as `by`: only the player's knocks pay (the bill, the boost, the heat, the chain). The walkers' step moved after the props' so a prop launched this step is dodged and its bound swept over the step before it moves; a knocked prop's body never dents a car (`Traffic`'s contact sums skip it), a post that holds is a wall.
87. **The save is version 5** (M8 slice 9, D10). `career.smashed` is the player's smashes, lifetime (Bernie's requirement reads it); a v4 document migrates with 0, and 0.7.0 treats a v5 save as newer (keeps it, plays without saving). Nothing of the props' state is saved: a new session's city stands whole.
88. **One purse: the road coins are in the bank; the save is version 6** (M8.5 slice 0, D1; DESIGN.md §17.4). Record 47's two pools were spent as one and shown as two numbers (BANK on the wall, CASH in the footer). A road coin's `coin` event adds to `Run.bank` (a spilled coin still to the bag); `Run.coins` and `Run.funds` are gone and every price checks the bank. A v5 document's `coins` is added to its `bank` (a broken field adds nothing) and v6 writes no `coins`; an older build treats a v6 save as newer (keeps it, plays without saving). `counts.coins` still counts the run's coins for the wall and the dailies.
89. **The boot's city build reads only what is near** (the M8.5 gate, M8's time to control). The props' rule measured every candidate against every segment of the authored roads near its chunk and every tall static, with an array a rectangle test; the road paint measured every stroke against every special road's whole polyline and found its segment by a scan from the start. Now the rule keeps, per chunk, each road's segments within its threshold's reach (a candidate inside the chunk's reach no wider than 8 m reads those; anything else reads all), rejects a tall static whose circle misses the footprint's before the rectangle test (a hair's margin keeps it on the safe side), and tests rectangles without an array; the paint skips a road whose box, grown by the margin, misses the point, and finds a segment by binary search. Every answer is the same to the last digit (the scratch comparison against the committed source: 246,414 characters of props and 3.8 million of paint equal); the props of 49 chunks 123–192 → 85–95 ms, the paint 155 → 77 ms, the world's build about 317 → 184–255 ms in Node. Time to control at 20 Mbit and 4× CPU: 0.7.0 3.91 s against 3.97 s (medians of five alternating pairs), from 4.32 against 4.69 before.
90. **A driving body turns about the vertical only; a shaken car drives on only on its wheels** (M8.6 slice 0, D1–D2; DESIGN.md §18). The height lock of record 31's deviation left roll and pitch free with no ground under the body: a push or the player's weight off its middle tipped a driving car and nothing righted it (a level-5 chase: 18 % of the driving samples over 15°, ten cars sunk up to 2.5 m, two on their roofs; the player on a car's tail tipped it to 67°). `lockDriving` locks both with the height, `unlock` frees all six (a shaken, wrecked, abandoned or parked car, and `disturb` now lets a driving car go too). The reattach no longer stands a car up from 45° in one step: level within 6°, rocking under 0.6 rad/s, within 15 cm of its road; one leaning at rest is rocked toward level every 0.5 s; overdue, a wreck.
91. **A traffic car's weight sits low; a stopped car keeps its pose and does not stand on an end** (M8.6 slice 1, D3–D4). The traffic's box gave its centre of mass in the collider's frame as if in the body's: three quarters up the box (1.05 m), a top-heavy car that rolled onto its side or roof at a touch; now `COM_SHARE` 0.38 of the box's height (a sedan's 0.53 m). Most of a chase's wrecks were those rolls: the bot's level-5 run went from 3 wrecks and 8 busts in 120 s to none (the balance at the gate). A wreck, an abandoned or a parked car's body writes its rotation and height into the record every step (`poseQ`, `y`, `posed`); body-less it is drawn so and lent again so, and `releaseBody` no longer moves it onto its lane. At rest on a side or an end for 0.5 s it is turned about the edge it rolls over (v = ω × (centre − edge), so the road does not stop the turn) onto its wheels, or its roof when it leans that way. The roadblock's braced car takes `carDamageFactor` 4 (3.5): the compact's 80 km/h hit reached exactly the wreck's 1.0 by the top-heavy car's rebound, 0.998 without it.
92. **The box goes round what stands there and does not shove** (M8.6 slice 2, D5–D7). A lent car moving at under a third of its command for 0.6 s while it touches something holds for 1.5 s (`heldFor`, `holdLeft`); a ram at a moving player is the one shove kept. A ram's velocity cap is on the shove, not the brakes: capped, a unit coming in at 24 m/s for a stopped player took 30 m to slow. The places are eight (the four and the diagonals); a place a unit would not fit on (the fixed ray, and any car but a unit on duty, its footprint grown by the unit's half width at three points along the player's heading) is shut and its slot takes a free diagonal, kept while free. A unit's line goes round the nearest slow car on it within 25 m, braked to the detour's pace by the time it is there, and every arrest approach is braked to the walking pace within `near` (v² = 7² + 2·6·(gap − 10)). A unit not yet dealt waits on the first standby place and is dealt next step. Under the busted card every unit holds where it is; for 20 s after, one that has not moved 2 m in 3 s goes back to the pool out of view.
93. **The officer never stands in a car** (M8.6 slice 3, D8). `TicketOfficer` picks the unit's door that is clear (the player's car counts), the driver's window unless a car stands within his half width of it (the passenger's, then behind the car), routes round the player's car by the corners of its footprint grown by 0.35 m when the straight way crosses it (a slab test, `segmentMeetsBox`; the window's corner of the nearer end when the way there is clear, the officer's before it), paces by that route's length, and after each walk is pushed out of every car's grown footprint along its nearer side (a walk into a car becomes a walk along it). Under the card the window's point is recomputed from the car every step. Walking straight to the door and left where he stood, he was inside the player's car on 6.5–20 % of his steps in the chase read; now none.
94. **A car in the camera's way is drawn through a screen door** (M8.6 slice 4, D9). `render/camera/fade.ts` decides (pure, pinned): a traffic car whose box (its rotation, so a wreck on its side too) grown by 0.4 m meets the line from the camera to the player's car, or stands within 2.5 m of the camera, heads for a quarter drawn, at the whole range per 0.15 s. `TrafficView` keeps a fade per record and writes it into an instanced `aFade` on each body's mesh; the traffic's material discards a fragment whose 4×4 Bayer threshold is at or over the fade (no blending: no sorting, no extra draw call, the shadow whole). A thinned police car shows no livery, bar or lenses (`PoliceView` skips it). The camera is the last frame's: the fade takes 9 frames, the frame's lag is not seen.
95. **The city is built ahead in idle time; the automatic quality reads the stutter** (M8.6 slice 5, D10). Measured on Marcin's laptop (i5-1035G1, MX330, headless Chromium on the real GPU, 1600×900): 0.8.5 and the candidate both held 60 fps in the bot's level-5 chase and its knocked street (the sim 1.2–1.8 ms a frame at p95, the renderer's JavaScript about 2 ms); a pile of ten cruisers and six burning wrecks round the stopped car ran 51–60 fps at a pixel ratio of 1 and, GPU-bound at 1.5, 47 fps with 49–65 frames over 50 ms on 0.8.5 (53–59 on the candidate, the thinned cars in the camera's way cost less); the same laptop swung the renderer's time 2–6 ms between runs as it heated. Two causes fixed: a first visit to a chunk generated it and its props in the step that needed it (5–35 ms, the slowest 40 ms in a 120 s drive): `City.prefetch` builds the 5×5 round the ring ahead, one piece a call, from the app's idle callback (pure and cached, so the sim is the same whenever it runs; the cache 32 chunks, it was 16), and the drive's slowest step is 6.3 ms; the automatic quality stepped down on the mean frame only (24 ms), and a stutter of 47 fps with a frame in five missed means 21 ms: `render/quality.ts` counts the frames over 22 ms and steps down at 8 % of them, up under 2 %.
96. **What the solid cars changed round them** (the M8.6 gate). The Chief comes back on its cadence even with the roster full, as one more (the roster used to thin by the wrecks the top-heavy cars made). An interceptor's PIT is two moves: alongside with its nose 1.2 m up the rear quarter, then a swerve aimed 0.8 m past the car's centre line over the rear wheel; aimed at the rear corner, a rigid car only sat on the bumper and pushed (the tilting bodies' bounces had been the blows). A civilian pressed by the player's car for 0.6 s yields for 1.5 s: no command, its speed worn off at 4/s, so a push moves it (a truck turning back against the bot's car held both till the bot gave up). The held-up rule is the units' only (a civilian holding in a junction's box kept the junction from the rest); a ram's brakes are uncapped only driving to a place round a stopped car (a ram or a PIT at a moving player keeps its tuned closing). The bot sees a car's ends in its way, and never passes a stopped car nor keeps passing one that stops.
97. **The renderer is split by the thing drawn** (2026-09-24, Marcin: "we could have a better structure"). Thirty-seven files in one folder became the frame at `src/render/` and eight folders. A folder takes the name of the sim folder it draws (`city`, `props`, `traffic`, `police`), so the sim's map is the renderer's; `cars/` holds every car's shape (the player's classes and the city's bodies share one loft, and a taken body is drawn with the player's mesh builder), `run/` the run's marks on the city (arrow, markers, coins, doors), `camera/` and `fx/` are the renderer's own. A new file goes into the folder of what it draws; the root takes only what every frame needs. Tests stay flat under `tests/render/`. `Renderer.ts` keeps only the frame's order (978 lines became 292): the player's car and its kit, the ghost, the camera's moments, the effects, the sky, the sim's shapes and the automatic quality's window each went to their own file.
98. **The screen's code is split by the screen it draws on** (2026-09-24, Marcin, after the renderer's split). Nineteen flat files became `hud/` (the driving screen), `map/` (the radar and the full map), `wall/` (the garage's wall), `dev/` (the tuning panel and the traces); the root keeps what every screen uses (the language, the styles) and the pause screen's settings. `totals` sits in `hud/` (the door's and the busted card's sums) and the wall's TOTALS page reads it from there. A new file goes into the folder of the screen it draws on (M9's touch layer into `hud/`). Tests stay flat under `tests/ui/`.
99. **The way is a field from the goal, not a search from the car** (M8.7 slice 0, D1–D2; DESIGN.md §20). `run/way.ts` keeps the goal the line names and the route the radar draws. The goal is `goalFor`'s precedence with the way as its chooser (`GoalChooser`): where it picks a ring or a door the pick is held, and moves only when another is under 0.6 of its road distance and 150 m nearer; a goal of another kind picks afresh. The road distances come from a forward pass from the car's lane every 0.5 s; the route from a reverse pass from the goal's lanes (the field: every lane's metres to the goal and its next lane), laid when the goal or a running job's phase changes and when a goal that moves has moved 10 m after a second, so a missed turn is a lookup. Both passes are the plain O(n²) Dijkstra over the 210 directed lanes (U-turn links included) into typed arrays made once, the junctions' curves measured by `LaneTables`; `city/route.ts`'s `laneChain` (allocating, lane lengths only) stays the cold open's and the bots'. The car's lane is the nearest running its way (within 8 m of the nearest of all; the last one kept at a junction within 2 m; a lane 5 m over or under skipped); a ring's, a door's or a point's lanes are the eight nearest within 25 m (a door's, its approach lane). Without a way (the test track) `goalFor` takes the straight line (`NEAREST`).
