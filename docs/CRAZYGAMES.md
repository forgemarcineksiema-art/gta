...
| # | Requirement | Source | Status | Notes |
|---|---|---|---|---|
| A1 | Never show a midgame ad while the player is actively playing; only at natural breaks (level transition, map change, after death, post-race summary). | requirements/ads | todo | Planned (docs/DESIGN.md §9): the hideout door after a banked run and the busted flow are the midgame points. The wrecked overlay is not one (a wreck forces a swap or a rolling respawn; the run continues). Never a takedown's slow motion nor the auto-pause on blur. One ad at most per door: the rewarded offer or the midgame request, not both. |
