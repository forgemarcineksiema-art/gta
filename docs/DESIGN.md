...
- **M8.10 The island** (decided by Marcin 2026-09-25 from the grid island's
  ...
  contract are one document, `docs/M8.10_PLAN.md`, §21 here a pointer): a
  plan drawn by hand, the rest from the seed; the hill under Crown
  Heights, the palms in Palm Gardens, the bay and the reef in Coral Quay,
  the highway round the coast with a tunnel, a viaduct and a bridge, the
  airfield and the mega-ramp to the islet, the stadium, the quarry, the
  drive-throughs. Beside M8.9, in its own worktree, behind `?map=island`
  until its switch.
...
The design and the contract are one file, `docs/M8.10_PLAN.md` (its §1 the
design, the sketches in `docs/island/`). In a line: the coast, the heights,
the districts, the main roads and every placed thing are drawn by hand
(`src/sim/island/plan.ts`); the districts' streets, the lots, the buildings
and the props still come from the seed. What it changes here: §6's layout,
roles and reward density read that §1, and §6.6's route is its first
minute (§1.4).
