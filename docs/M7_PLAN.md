# M7 "Platform" — implementation plan

Renumbered 2026-09-23 (Marcin): this was M6 until then, the brief's M6;
M6 is now the wanted board (`docs/M6_PLAN.md`, DESIGN §14), and older docs
that say M6 for the platform mean this milestone. What M6 hands over: the
STYLE page's day's pick (one known kit item a day at half price) becomes a
rewarded offer here, free for a video with the half price as the equal
alternative (slice 4); the horn (H) wants a touch button beside boost
(slice 2); the save is version 3 (slice 0 checks it through the Data
module). The perf bases below are the M6 gate's wherever they say M5's.

Executor: the agent that starts after the M6 gate. Reviewer: Claude, at the
...
Brief §9 (its M6, this M7): *"M6 Platform. `CrazyGamesPlatform`, full compliance checklist,
...
`npm run perf:mobile`; `docs/M7_REPORT.md` per `CLAUDE.md` including the
...
PROGRESS entries per session as before. `docs/M7_REPORT.md`: built, verify
...
8. `docs/M7_REPORT.md`, ARCHITECTURE records, BACKLOG, PROGRESS.
...
- Docs: ARCHITECTURE records, STYLE (the touch layer and the overlay),
  ...
  `M7_REPORT.md` with the submission steps.
