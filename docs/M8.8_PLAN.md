...
after M8.7's gate and before M9, as every milestone since M5.5 (DESIGN §11:
the platform starts when all of it is in). It was first written as M10,
after the launch; only its place and its number changed. The platform keeps
its number and its contract, `docs/M9_PLAN.md`, and starts after this gate.
...
`npm run verify:gate`; `perf` against M8.7's gate build; `game`, `heat`,
...
Polish and in English. Report `docs/M8.8_REPORT.md`; 0.8.8.
...
Bases: M8.7's gate build, `npm run perf` twice. Expected costs: a mesh per
