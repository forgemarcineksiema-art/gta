...
- **M8.9 is built beside it** (`CLAUDE.md`, Two milestones at once). M8.9
  ...
  14.1). Before a slice that touches a file M8.9 also changes, merge main in.
...
Run with M8.9's session idle (Marcin's word) and Marcin's game closed
(Playwright reuses whatever answers on 4173). First, if M8.9's gate has not
...
- **M8.9 changes the same screen files** (`src/ui/wall/garage.ts`, the maps,
  ...
  `Sfx`): merge main into `m8.8-fleet` right before a slice that touches one
  (`CLAUDE.md`, Two milestones at once).
- **Playwright reuses whatever answers on 4173** (`reuseExistingServer`): an
  e2e run in the worktree with Marcin's game open measures main's build, not
  this branch's.
