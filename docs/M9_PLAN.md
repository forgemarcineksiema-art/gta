...
down), the save's version 5, and the perf bases of the M8 gate wherever this
file says M5's or M7's. What M6 hands over: the STYLE page's day's pick (one
known kit item a day at half price) becomes a rewarded offer here, free for
a video with the half price as the equal alternative (slice 4); the horn (H)
wants a touch button beside boost (slice 2). What M7 hands over: the
...
decision carries its reason. Written 2026-09-22 against commit `ba9a12f`;
the SDK facts it relies on were read from docs.crazygames.com on 2026-09-20
(`docs/CRAZYGAMES.md`) and slice 0 re-reads them before any adapter code
exists.
