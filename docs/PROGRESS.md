...
- **Car model rebuilt as a parametric loft** (`src/render/carMesh.ts`, per the brief's "built in code from parametric profiles"): cross-sections with floor / belt / roof heights and widths give a low nose, bonnet, raked windscreen, narrow greenhouse, rear window and tail; faces coloured per region and wound outward automatically; details (bumpers, lights, grille, mirrors, spoiler, exhausts) are boxes; wheels sit outside the tucked sills. `MUSCLE` is the first `CarProfile`; the other seven classes will be profiles too.
- 29 sim tests.
