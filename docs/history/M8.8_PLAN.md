...
Everything in `docs/history/M5_PLAN.md` §1.4. Nothing here adds a break the game
...
SPEED, DISGUISE (DRIFT, MIASTO, TARAN, PRĘDKOŚĆ, PRZEBRANIE; DRIFT and MIASTO
are the words the combo and the boot already say, one name each); a civilian
...
the budget is never loosened. (Marcin, 2026-09-25: "włącz fizyczne radiowozy i
rywali"; they are on, and a failed budget goes to him with its numbers instead of switching them off.)
