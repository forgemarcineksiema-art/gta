...
export class Traffic {
  ...
  private placeAtPoint(i: number, x: number, z: number, yaw: number, body: number, state: AgentState, paint: number, y?: number): void {
    this.state[i] = state;
    this.parkedCiv[i] = 0;
    this.parkBay[i] = -1;
    this.racer[i] = 0;
    ...
    this.paintSerial++;
    // on the ground where it stands (the grid's 0; a bay on the island's hill, nose up or down its slope), or level at
    // the height given (a roof)
    const fx = Math.sin(yaw) * 2, fz = Math.cos(yaw) * 2;
    this.y[i] = y ?? this.streets.groundAt(x, z);
    this.grade[i] = y === undefined ? (this.streets.groundAt(x + fx, z + fz) - this.streets.groundAt(x - fx, z - fz)) / 4 : 0;
    this.posed[i] = 0;
    this.tipFor[i] = 0;
    const q = M.quatSetAxisAngle(this.scratchQ, 0, 1, 0, yaw);
