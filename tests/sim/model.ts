...
export interface Knobs {
  ...
  fine: number;
  prices: { compact: number; heavy: number; moto: number; offroad: number; sports: number };
 ...
 * them (M8.8 slice 10) and the motorbike after it (slice 16: straight after the van, the 4×4 waited 10.5 minutes), the
 * sports car last (the second hour's goal; the tiers carry the first).
 */
export function ladder(knobs: Knobs = BALANCE_KNOBS): Array<[string, number]> {
  ...
  return [
    ...
    ['offroad', knobs.prices.offroad], ['moto', knobs.prices.moto],
...
export function firstHour(input: ModelInput, knobs: Knobs = BALANCE_KNOBS): FirstHour {
  ...
  const seen: number[] = [];
  const CARS = new Set(['compact', 'heavy', 'moto', 'offroad', 'sports']);
